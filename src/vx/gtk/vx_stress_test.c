...
static void draw(state_t * state, vx_world_t * world)
{
    ...
    if (state->img != NULL){
        ...
        vx_object_t * o3 = vxo_image(vx_resc_copyub(img->buf, img->width*img->height*img->bpp),  img->width, img->height, img->bpp == 4? GL_RGBA : GL_RGB, VXO_IMAGE_FLIPY);
...
void * render_loop(void * data)
{
    ...
    while(state->running) {
        double rad = (tinfo->id*M_PI/5) + (vx_mtime() % 2000) * 2* M_PI / 1e3;
...
void * camera_loop(void * data)
{
    ...
    while (state->running) {
        ...
        double rad = ( (vx_mtime() - start_mtime) % 5000) * 2* M_PI / 5e3;
