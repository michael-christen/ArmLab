...
static inline void memcpy64(uint8_t *dest, const uint8_t *src)
{
    if (sizeof(void*) == 8) {
        memcpy(dest, src, 8);
    } else {
...
static inline int uc5_bit(struct uc5_state *state)
{
...
static inline int uc5_bits(struct uc5_state *state, int nbits)
{
...
static inline uint32_t uc5_varint(const uint8_t *in, int *inpos)
{
    uint32_t v = 0;
    int shift = 0;
...
static inline void uc5_literal(struct uc5_state *state)
{
    uint32_t len;
...
static inline void uc5_copy(struct uc5_state *state)
{
    uint32_t len, ago;
...
static inline uint32_t c5_hash(struct c5_state *state, int inpos)
{
...
static inline void c5_bit_flush(struct c5_state *state)
{
    for (int i = 0; i < sizeof(bits_t); i++) {
...
static inline void c5_bit(struct c5_state *state, uint64_t bit)
{
    assert(bit == 0 || bit == 1);
...
static inline void c5_bits(struct c5_state *state, uint64_t bits, int nbits)
{
...
static inline void c5_varint(uint8_t *out, int *outpos, uint32_t v)
{
  more:
...
static inline void c5_update_history(struct c5_state *state, int inpos)
{
...
static inline void c5_literal(struct c5_state *state)
{
...
static inline void c5_copy(struct c5_state *state)
{
