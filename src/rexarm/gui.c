...
void* render_arm(void* data){
	...
	while(gstate->running){
		...
		vx_object_t *block6 = vxo_chain(vxo_mat_rotate_x(view_above*M_PI/2),
		...
		vx_object_t *claw = vxo_chain(vxo_mat_rotate_x(view_above*M_PI/2),
...
void* gui_create(int argc, char **argv){
	...
	if(!strcmp(getopt_get_string(gstate->gopt, "mode"),"click")){
		...
		printf("Mode '%s' format not recognized. Use 'click', 'view', or 'camera'\n", getopt_get_string(gstate->gopt, "mode"));
		exit(1);
	}
