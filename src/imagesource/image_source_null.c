...
static void fill_solid(struct null_format *nf, image_source_data_t *isd)
{
    if (!strcmp(nf->format, "GRAY8"))
        isd->datalen = nf->width * nf->height;
    else if (!strcmp(nf->format, "RGB"))
...
static void fill_random(struct null_format *nf, image_source_data_t *isd)
{
    if (!strcmp(nf->format, "GRAY8")) {
        isd->datalen = nf->width * nf->height;
    } else if (!strcmp(nf->format, "RGB")) {
...
image_source_t *image_source_null_open(url_parser_t *urlp)
{
    ...
    impl_null_t *impl = calloc(1, sizeof(impl_null_t));
    isrc->impl = impl;
    ...
    if (1) {
        struct null_format *nf = calloc(1, sizeof(struct null_format));
        nf->width = 640;
        nf->height = 480;
